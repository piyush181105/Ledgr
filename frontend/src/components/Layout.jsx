...
const Layout = ({ onLogout, user }) => {
  ...
  const fetchTransactions = async () => {
    try {
      setLoading(true);
      // This ensures we check BOTH places and don't just settle for a boolean
      ...
      console.log("DEBUG: Current Token is ->", token);
  ...
  // Replace the useEffect at line 177 with this:
  useEffect(() => {
    const token = localStorage.getItem("token") || sessionStorage.getItem("token");
    console.log("LAYOUT CHECK: ", token); // See what the layout sees

    if (token && token !== "true") {
      fetchTransactions();
    } else {
      // Only redirect if we are sure we aren't logging in
      const timeout = setTimeout(() => {
        if (!localStorage.getItem("token") && !sessionStorage.getItem("token")) {
          navigate("/login");
        }
      }, 500); // Give the app half a second to save the token
        ...
        <div className={styles.statCards.grid}>
          <div className={styles.statCards.card}>
            ...
            <p className={styles.statCards.cardfooter}>
              <span className="text-teal-600 font-mediumn">
          ...
          <div className={styles.statCards.card}>
            ...
            <p className={styles.statCards.cardfooter}>
              <span className="text-green-600 font-mediumn">
                +12.5%
              </span>{" "}
              from last month
            </p>
                      ...
                      <div className="flex items-center gap-1 md:gap-4 lg:gap-3">
                        ...
                        <div className={styles.transactions.details}>
                          ...
                          <p className={styles.transactions.meta}>
                            ...
                            <span className="ml-2 capiitalize">{category}</span>
                          </p>
                        </div>
                      </div>
